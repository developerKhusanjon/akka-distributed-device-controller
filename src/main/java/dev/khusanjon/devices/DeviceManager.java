...
public class DeviceManager extends AbstractBehavior<DeviceManager.Command> {
    ...
    private static class DeviceGroupTerminated implements DeviceManager.Command {
        ...
        private DeviceGroupTerminated(String groupId) {
            this.groupId = groupId;
        }
    }
    ...
    private DeviceManager onRequestDeviceList(RequestDeviceList rdl) {
        ActorRef<DeviceGroup.Command> ref = groupIdToActor.get(rdl.groupId);
        if (ref != null)
            ref.tell(rdl);
        else
            rdl.replyTo.tell(new ReplyDeviceList(rdl.requestId, Collections.emptySet()));
        return this;
    }
